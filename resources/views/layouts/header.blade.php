        ...
        <div class="bookmark-wrapper d-flex align-items-center">
            ...
            <ul class="nav navbar-nav align-items-center ml-auto">
                @foreach($app_moduls as $key => $modul)
                    @php
                        $firstMenu = DB::table('app_menus')
                        ->select('app_menus.target')
                        ->join('app_user_accesses', function ($join) use ($modul) {
                            $join->on('app_menus.id', 'app_user_accesses.menu_id');
                            $join->where('app_user_accesses.name', 'index');
                            $join->where('app_menus.status', 't');
                            $join->where('app_menus.modul_id', $modul->id);
                            $join->on('app_user_accesses.group_id', DB::raw(\Auth::user()->group_id));
                        })
                        ->join('app_sub_moduls', 'app_menus.sub_modul_id', 'app_sub_moduls.id')
                        ->orderBy('app_sub_moduls.order', 'asc')
                        ->orderBy('app_menus.order', 'asc')
                        ->get()->first();
                    @endphp
                    <li class="nav-item mr-50 pb-25">
                        <a class="nav-link dropdown-toggle align-items-center" href="{{ !empty($firstMenu) ? route($modul->target.'.'.$firstMenu->target.'.index') : '#' }}">
                            <i class="{{ $modul->icon ?? 'fa fa-folder' }} mt-50 mr-50"
                               style="{!! $prefix == $modul->target ? 'color: '.$maincolor : '' !!}" aria-hidden="true"></i>
                            <span class="selected-language mb-50" style="font-size: 15px;{{ $prefix == $modul->target ? "color: ".$maincolor : '' }}">{{ $modul->name }}</span>
                        </a>
                    </li>
                @endforeach
            </ul>
