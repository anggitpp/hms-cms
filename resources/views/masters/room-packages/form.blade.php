...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form id="form" enctype="multipart/form-data" method="POST" action="{{ empty($package) ? route('masters.room-packages.store', $id) : route('masters.room-packages.update', $package->id) }}">
                @csrf
                @if(!empty($package))
                    @method('PATCH')
                @endif
